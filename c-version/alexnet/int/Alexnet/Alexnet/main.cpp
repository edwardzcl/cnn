...
int**** product_weightmatrix(int *initA, int m, int n, int p, int q)  //传文件指针是可以的，并且作为一种特殊的指针会接上次的调用
{
	int i, j, k, l;
	int**** arr_b;
...
int **product_fcweightmatrix(int *initA, int mw, int nw)
{
	int i, j;
	int** arr_fcweight;
...
int main()
{
	...
	*/
	...
	pmatFile = matOpen("weight.mat", "r");

	//int M = mxGetM(pMxArray);
	//int N = mxGetN(pMxArray);


	pMxArray1 = matGetVariable(pmatFile, "weight1");
	pMxArray2 = matGetVariable(pmatFile, "weight2");
	pMxArray3 = matGetVariable(pmatFile, "weight3");
	pMxArray4 = matGetVariable(pmatFile, "weight4");
	pMxArray5 = matGetVariable(pmatFile, "weight5");
	pMxArray6 = matGetVariable(pmatFile, "weight6");
	pMxArray7 = matGetVariable(pmatFile, "weight7");
	pMxArray8 = matGetVariable(pmatFile, "weight8");

	initA = (int*)mxGetData(pMxArray1);
	arr_w1 = product_weightmatrix(initA, 11, 11, 3, 96);
	mxFree(initA);

	initA = (int*)mxGetData(pMxArray2);
	arr_w21 = product_weightmatrix(initA, 5, 5, 48, 128);
	initB = &initA[153600];
	arr_w22 = product_weightmatrix(initB, 5, 5, 48, 128);
	mxFree(initA);

	initA = (int*)mxGetData(pMxArray3);
	arr_w3 = product_weightmatrix(initA, 3, 3, 256, 384);
	mxFree(initA);

	initA = (int*)mxGetData(pMxArray4);
	arr_w41 = product_weightmatrix(initA, 3, 3, 192, 192);
	initB = &initA[331776];
	arr_w42 = product_weightmatrix(initB, 3, 3, 192, 192);
	mxFree(initA);

	initA = (int*)mxGetData(pMxArray5);
	arr_w51 = product_weightmatrix(initA, 3, 3, 192, 128);
	initB = &initA[221184];
	arr_w52 = product_weightmatrix(initB, 3, 3, 192, 128);
	mxFree(initA);

	initA = (int*)mxGetData(pMxArray6);
	arr_w6 = product_fcweightmatrix(initA, 4096, 9216);
	mxFree(initA);

	initA = (int*)mxGetData(pMxArray7);
	arr_w7 = product_fcweightmatrix(initA, 4096, 4096);
	mxFree(initA);

	initA = (int*)mxGetData(pMxArray8);
